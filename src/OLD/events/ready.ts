        ...
        const autoMarket = new CronJob(
            ...
            function marketUpdate() {
                ...
                connection.query(sql, async(err, results) => {if (err) { throw err; }
                    ...
                    writeFileSync('src/OLD/data/prix.json', jsonPrix)

                    jsonPrix = JSON.parse(readFileSync('src/OLD/data/prix.json', 'utf-8'));
                    const embed = {
            ...
            function competitionUpdate() {
                ...
                connection.query(sql, async(err, results) => {if (err) {throw err;}
                    ...
                    function monClassement1(item: { id_joueur: any; habitant: number; ancien_pop: number; }) {
                        number += 1;
                        const croissanceDemo = item.habitant - item.ancien_pop;
                        const emoji = emojiCroissanceDemo(croissanceDemo);
                        const element = {
                            name: `#${number}`,
                            value: `<@${item.id_joueur}>\n` +
                                codeBlock(`• ${emoji} ${croissanceDemo.toLocaleString('en-US')} habitants`) + `\u200B`}
                        array1.push(element);
