...
var _PaginatedMessage = class {
  constructor({
    ...
    __publicField(this, "hasEmittedWarning", false);
    this.pages = pages ?? [];
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new import_discord3.InteractionCollector(targetUser.client, {
        filter: (interaction) => !(0, import_utilities5.isNullish)(this.response) && interaction.message.id === this.response?.id && interaction.isMessageComponent() && this.actions.has(interaction.customId),
        time: this.idle,
